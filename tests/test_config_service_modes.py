...
class ConfigServiceModeTests(unittest.TestCase):
    def test_split_service_only_config_is_supported_without_run_settings(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            (root / "workspace").mkdir(parents=True)
            config_path = write_config(root, {"service": default_service_block()})
    ...
    def test_env_file_overrides_service_settings_only(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            (root / "runtime.env").write_text("SERVICE_PORT=9015\nQUEUE_BACKEND=redis", encoding="utf-8")
            payload = {"env_file": "runtime.env", "service": default_service_block()}
    ...
    def test_precedence_defaults_env_json_cli(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            payload = {
                ...
                "service": {"mount_work_root": "workspace", "port": 9010, "queue_backend": "inmemory"},
                "run": {
                    "model": "test-model",
                    "provider_url": "http://localhost:11434",
                    "project_root": "workspace/project",
            ...
            cfg = load_config(write_config(root, payload), overrides={"service_port": 9020, "queue_backend": "inmemory"})
