...
class CliShellDetectionTests(unittest.TestCase):
    def test_shell_auto_detection_prefers_powershell_when_pwsh_missing(self) -> None:
        which_table = {
            "pwsh": None,
        ...
        with (
            ...
            patch("lllars_core.shell.shutil.which", side_effect=lambda name: which_table.get(name)),
        ):
    ...
    def test_shell_override_is_honored_when_available(self) -> None:
        with (
            ...
            patch(
                ...
                side_effect=lambda name: "C:/Windows/System32/cmd.exe" if name == "cmd" else None,
            ),
        ):
            selection = detect_shell(shell_mode="override", shell_override="cmd")
        self.assertIsNotNone(selection)
    ...
    def test_shell_override_is_used_when_mode_is_auto(self) -> None:
        ...
        with (
            patch("lllars_core.shell.platform.system", return_value="Windows"),
            patch("lllars_core.shell.shutil.which", side_effect=lambda name: table.get(name)),
        ):
