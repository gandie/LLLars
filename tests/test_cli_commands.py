...
class CliCommandTests(unittest.TestCase):
    def test_oneshot_requires_prompt_or_prompt_file(self) -> None:
        cfg = base_run_cfg()
        with (
            patch.object(sys, "argv", ["lllars", "--config", "playground.example.json"]),
            patch("lllars_core.cli.load_config", return_value=cfg),
        ):
            with self.assertRaisesRegex(SystemExit, "Provide --prompt or --prompt-file"):
                main()
    ...
    def test_oneshot_uses_run_job_and_returns_success_exit_code(self) -> None:
        ...
        self.assertEqual(call.args[0].config_path, str(Path("playground.example.json").resolve()))
        print_summary.assert_called_once()

    def test_serve_uses_runtime_app_with_uvicorn(self) -> None:
        cfg = type("Cfg", (), {"queue_backend": "inmemory", "service_host": "127.0.0.1", "service_port": 8123, "service_workers": 1})
        with (
            patch.object(
                sys,
                "argv",
            ...
            patch("lllars_core.cli.print_runtime_startup"),
            patch("lllars_core.cli._run_startup_preflight"),
            patch("lllars_core.cli.create_runtime_app", return_value="app") as create_app,
            patch("uvicorn.run") as uvicorn_run,
        ):
            main()
        create_app.assert_called_once_with(cfg)
        uvicorn_run.assert_called_once_with("app", host="127.0.0.1", port=8123, log_level="info")

    def test_serve_rejects_unsupported_queue_backend(self) -> None:
        cfg = type("Cfg", (), {"queue_backend": "redis", "service_host": "127.0.0.1", "service_port": 8000, "service_workers": 1})
        with (
            patch.object(
                sys,
                "argv",
                ["lllars", "serve", "--config", "playground.example.json", "--queue-backend", "redis"],
            ),
            patch("lllars_core.cli.load_config", return_value=cfg),
            patch("lllars_core.cli.print_runtime_startup"),
            patch("lllars_core.cli._run_startup_preflight"),
        ):
            with self.assertRaisesRegex(SystemExit, "Serve mode currently supports only queue_backend=inmemory"):
                main()
