...
class CliRegressionTests(unittest.TestCase):
    def test_shell_auto_detection_prefers_pwsh_then_powershell_then_cmd(
        self,
    ) -> None:
        which_table = {
            "pwsh": None,
    ...
    def test_shell_override_is_honored_when_available(self) -> None:
        with (
            ...
            patch(
                "lllars_core.shell.shutil.which",
                side_effect=lambda name: "C:/Windows/System32/cmd.exe"
                if name == "cmd"
                else None,
            ),
        ):
    ...
    def test_shell_override_falls_back_to_auto_when_mode_is_auto(self) -> None:
        with (
            patch("lllars_core.shell.platform.system", return_value="Windows"),
            patch(
                "lllars_core.shell.shutil.which",
